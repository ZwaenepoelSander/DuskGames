...
export function getDetectExternalImportsPlugin(options, logicPath) {
    ...
    function getExternalDeps(skipAllowedDependencies) {
        ...
        nodes.forEach((node) => {
            const name = node.model.fileName;
            //Not external, ignore it
            if (path.isAbsolute(name)) {
                return;
            }
            const isInWhitelist = DEPENDENCY_WHITELIST.includes(name) ||
    ...
    return [
        {
            ...
            transform: async (code, idWithExtension) => {
                try {
                    ...
                    const transformedFileNode = treeNodesByFile[id] || logicImportTree.parse({ fileName: id });
